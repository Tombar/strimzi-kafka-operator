...
public class ZookeeperSetOperatiorTest {
    ...
    public void before() {
        ...
        a = ZookeeperCluster.fromCrd(getResource(), getInitialSecrets(getResource().getMetadata().getName())).generateStatefulSet(true);
        b = ZookeeperCluster.fromCrd(getResource(), getInitialSecrets(getResource().getMetadata().getName())).generateStatefulSet(true);
    }
    ...
    private Certificates getInitialSecrets(String clusterName) {
        String clusterCmNamespace = "test";
        return ResourceUtils.createInitialCertificates(clusterCmNamespace, clusterName);
    }
